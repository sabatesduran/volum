...
export function RestoreBackupDialog({ prepared, onClose }: { prepared: PreparedRestore; onClose: () => void }) {
  ...
  return <Dialog title={t("Restore this backup?")} subtitle={t("Volum validated the archive and will make a safety backup of the current app data first.")} onClose={() => { if (!restoring) void cancel(); }} size="medium">
    ...
    <div className="restore-counts"><div><strong>{prepared.counts.models.toLocaleString()}</strong><span>{t("Models")}</span></div><div><strong>{prepared.counts.collections.toLocaleString()}</strong><span>{t("Collections")}</span></div><div><strong>{prepared.counts.tags.toLocaleString()}</strong><span>{t("Tags")}</span></div><div><strong>{prepared.counts.webSources.toLocaleString()}</strong><span>{t("Web imports")}</span></div></div>
