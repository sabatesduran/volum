...
export function Onboarding({ onComplete }: { onComplete: () => void }) {
  ...
  const choose = async () => {
    const path = isTauri() ? await open({ directory: true, multiple: false, title: t("Choose your 3D project library") }) : "/Users/you/3D Projects";
  ...
  return (
    <div className="onboarding">
      ...
      <div className="onboarding__copy"><div className="eyebrow">{t("Welcome to Volum")}</div><h1>{t("A beautiful home")}<br />{t("for your 3D projects.")}</h1><p>{t("Choose one or more folders. Volum indexes them locally without moving or uploading a single file.")}</p><div className="onboarding__promises"><span><LockKeyhole size={17} /><span><strong>{t("Private by design")}</strong><small>{t("No account or cloud required")}</small></span></span><span><Sparkles size={17} /><span><strong>{t("Your folders, enriched")}</strong><small>{t("Automatic previews and metadata")}</small></span></span></div><div className="onboarding__actions"><button className="button button--primary button--large" onClick={choose}><FolderPlus size={18} /> {t("Choose a project folder")}</button><button className="button button--quiet button--large" disabled={restoring} onClick={() => void chooseBackup()}>{restoring ? <LoaderCircle className="spin" size={18} /> : <ArchiveRestore size={18} />} {t(restoring ? "Validating backup…" : "Restore a backup…")}</button></div>{restoreError && <div className="form-error">{restoreError}</div>}<small className="onboarding__footnote">{t("Supports STL, 3MF, OBJ, STEP, and STP files")}</small></div>
