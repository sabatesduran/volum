...
import { formatBytes, formatCurrency, formatDate, formatDimensions, formatDuration, titleCase } from "../../lib/format";
...
function OpenInSlicer({ assetId, disabled, applications, defaultId }: { assetId?: string; disabled: boolean; applications: SlicerApplication[]; defaultId?: string }) {
  ...
  return <div className="open-slicer" ref={root}><button className="open-slicer__main" disabled={disabled} onClick={() => launch(primary)}>{primary ? <SlicerIcon application={primary} size={23} /> : <ExternalLink size={17} />}<span>{primary ? `Open in ${primary.name}` : "Open in default app"}</span></button><button className="open-slicer__toggle" disabled={disabled} onClick={() => setMenuOpen((open) => !open)} aria-label="Choose slicer" aria-expanded={menuOpen}><ChevronDown size={16} /></button>{menuOpen && <div className="open-slicer__menu"><div className="open-slicer__menu-label">Open with</div>{applications.map((application) => <button key={application.id} onClick={() => launch(application)}><SlicerIcon application={application} size={25} /><span><strong>{application.name}</strong><small>{application.id === defaultId ? "Default slicer" : "Configured slicer"}</small></span>{application.id === defaultId && <Check size={14} />}</button>)}{applications.length > 0 && <div className="open-slicer__separator" />}<button onClick={() => launch()}><span className="open-slicer__system"><ExternalLink size={14} /></span><span><strong>System default</strong><small>Use the operating system association</small></span></button></div>}</div>;
}
  ...
  return (
    <div className="cost-calculator">
      <div className="calculator-grid">
        <label className="field field--compact"><span>Plastic used</span><div className="input-unit"><input type="number" min="0" step=".01" value={grams} placeholder="Enter weight" onChange={(event) => { setGrams(event.target.value); setUsageSource("manual"); }} /><span>g</span></div><small>{usageSource === "file" ? "From sliced 3MF" : usageSource === "web" ? "From the default web print profile" : grams ? "Manual value" : "Not available in this file"}</small></label>
        <label className="field field--compact"><span>Quantity</span><input type="number" min="1" value={quantity} onChange={(event) => setQuantity(Math.max(1, Number(event.target.value)))} /></label>
      </div>
      {materials.length > 0 && <label className="field field--compact"><span>Material</span><select value={materialId || defaultMaterial?.id} onChange={(event) => setMaterialId(event.target.value)}>{materials.map((material) => <option key={material.id} value={material.id}>{material.name} · {formatCurrency(material.spoolPriceMinor, material.currency)}</option>)}</select></label>}
      <div className="calculator-grid">
        <label className="field field--compact"><span>Spool price</span><div className="input-unit input-unit--prefix"><span>{currency === "EUR" ? "€" : currency === "USD" ? "$" : currency}</span><input type="number" min="0" step=".01" value={price} onChange={(event) => setPrice(Number(event.target.value))} /></div></label>
        <label className="field field--compact"><span>Spool weight</span><div className="input-unit"><input type="number" min="1" value={weight} onChange={(event) => setWeight(Number(event.target.value))} /><span>g</span></div></label>
      </div>
      <div className="cost-result"><div><span>One piece</span><strong>{calculated ? formatCurrency(calculated.costPerPieceMinor, currency) : "—"}</strong></div><div className="cost-result__batch"><span>Batch · {quantity}</span><strong>{calculated ? formatCurrency(calculated.batchCostMinor, currency) : "—"}</strong></div></div>
      <button className="button button--quiet button--full" disabled={!calculated} onClick={save}><Save size={15} /> Save estimate</button>
    </div>
  );
}
...
function CollectionPicker({ modelId, selected }: { modelId: string; selected: string[] }) {
  ...
  if (!manual.length) return <p className="quiet-copy">Create a manual collection to organize this model without moving its files.</p>;
...
function TagEditor({ modelId, selected }: { modelId: string; selected: Array<{ id: string; name: string; color: string }> }) {
  ...
  return <div className="tag-editor"><div className="tag-chips">{selected.map((tag) => <button key={tag.id} style={{ "--tag-color": tag.color } as React.CSSProperties} onClick={() => toggle(tag.id)}>{tag.name}<X size={11} /></button>)}{!selected.length && <span className="quiet-copy">No tags yet.</span>}</div><form onSubmit={create} className="tag-add"><input value={name} onChange={(event) => setName(event.target.value)} list={`tags-${modelId}`} placeholder="Add or create a tag…" maxLength={40} /><datalist id={`tags-${modelId}`}>{tags.filter((tag) => !selected.some((item) => item.id === tag.id)).map((tag) => <option key={tag.id} value={tag.name} />)}</datalist><button type="submit" className="icon-button" disabled={!name.trim()} aria-label="Add tag"><Plus size={15} /></button></form>{tags.some((tag) => !selected.some((item) => item.id === tag.id)) && <div className="tag-suggestions">{tags.filter((tag) => !selected.some((item) => item.id === tag.id)).slice(0, 8).map((tag) => <button key={tag.id} onClick={() => toggle(tag.id)}><span style={{ background: tag.color }} />{tag.name}</button>)}</div>}</div>;
}
...
function PlateThumbnail({ assetId, plate }: { assetId: string; plate: ThreeMfPlate }) {
  ...
  return <div className="plate-thumbnail">{source ? <img src={source} alt={`Plate ${plate.index}`} /> : <span><Box size={18} />Plate {plate.index}</span>}</div>;
}
...
function ThreeMfInspector({ assetId, metadata, selectedPlateIndex, onSelectPlate, onShowAll }: { assetId: string; metadata: ThreeMfMetadata; selectedPlateIndex?: number; onSelectPlate: (index: number) => void; onShowAll: () => void }) {
  ...
  return <div className="three-mf-inspector"><dl className="metadata-list metadata-list--profile">{metadata.printer && <div><dt>Printer</dt><dd>{metadata.printer}</dd></div>}{metadata.printProfile && <div><dt>Profile</dt><dd>{metadata.printProfile}</dd></div>}{metadata.nozzleDiameterMm != null && <div><dt>Nozzle</dt><dd>{metadata.nozzleDiameterMm} mm</dd></div>}{metadata.layerHeightMm != null && <div><dt>Layer height</dt><dd>{metadata.layerHeightMm} mm</dd></div>}{metadata.slicer && <div><dt>Created with</dt><dd>{metadata.slicer}{metadata.slicerVersion ? ` · ${metadata.slicerVersion}` : ""}</dd></div>}</dl>{plates.length > 0 ? <div className="plate-list"><button type="button" className={`plate-card plate-card--all ${selectedPlateIndex == null ? "is-active" : ""}`} onClick={onShowAll}><span className="plate-thumbnail plate-thumbnail--all"><Box size={20} /></span><span className="plate-card__copy"><span className="plate-card__title"><strong>All plates</strong><span>{plates.length} plates</span></span><p>Show the complete 3MF project</p></span></button>{plates.map((plate) => { const objectNames = plate.objectNames ?? []; return <button type="button" className={`plate-card ${selectedPlateIndex === plate.index ? "is-active" : ""}`} key={plate.index} onClick={() => onSelectPlate(plate.index)}><PlateThumbnail assetId={assetId} plate={plate} /><div className="plate-card__copy"><div className="plate-card__title"><strong>{plate.name || `Plate ${plate.index}`}</strong><span>{plate.objectCount} {plate.objectCount === 1 ? "object" : "objects"}</span></div>{objectNames.length > 0 && <p>{objectNames.slice(0, 3).join(" · ")}{objectNames.length > 3 ? ` +${objectNames.length - 3}` : ""}</p>}<div className="plate-facts">{plate.printTimeSeconds != null && <span><Clock3 size={11} />{formatDuration(plate.printTimeSeconds)}</span>}{plate.filamentGrams != null && <span><Scale size={11} />{plate.filamentGrams.toFixed(1)} g</span>}{(plate.materialNames ?? []).map((material) => <span key={material}>{material}</span>)}{plate.bedType && <span>{plate.bedType}</span>}</div></div></button>; })}</div> : <p className="quiet-copy">This 3MF contains a model but no plate definitions.</p>}</div>;
}
...
export function ModelDetailView({ modelId }: { modelId: string }) {
  ...
  if (isLoading) return <div className="detail-loading"><LoaderCircle className="spin" /><span>Preparing model…</span></div>;
  if (error || !model) return <div className="empty-state"><TriangleAlert /><h2>Couldn’t open this model</h2><p>{error instanceof Error ? error.message : "The model may have moved."}</p><button className="button" onClick={() => selectModel()}>Back to library</button></div>;

  return (
    <div className="model-detail">
      <section className="model-detail__stage">
        <div className="detail-toolbar">
          <button className="button button--floating" onClick={() => selectModel()}><ArrowLeft size={17} /> Back</button>
          <div className="detail-toolbar__right">
            <button className={`button button--floating button--icon ${model.favorite ? "is-favorite" : ""}`} onClick={() => favorite.mutate()} aria-label="Favorite"><Heart size={17} fill={model.favorite ? "currentColor" : "none"} /></button>
            <button className="button button--floating" onClick={() => { setCameraView("iso"); setResetSignal((value) => value + 1); }}><RotateCcw size={16} /> Reset view</button>
          </div>
        </div>
        <div className="viewer-wrap">
          ...
          {viewerPlates.length > 1 && <div className="viewer-plate-controls" role="group" aria-label="3MF plate"><button type="button" className={activePlateIndex == null ? "is-active" : ""} onClick={() => setActivePlateIndex(undefined)}>All plates</button>{viewerPlates.map((plate) => <button type="button" key={plate.index} className={activePlateIndex === plate.index ? "is-active" : ""} onClick={() => setActivePlateIndex(plate.index)}>{plate.name || `Plate ${plate.index}`}</button>)}</div>}
          <div className="viewer-view-controls" role="group" aria-label="Model view">{(["iso", "front", "side", "top"] as ModelView[]).map((view) => <button type="button" key={view} className={cameraView === view ? "is-active" : ""} onClick={() => setCameraView(view)}>{view}</button>)}</div>
      ...
      <aside className="model-detail__inspector">
        <header className="inspector-header">
          <div className="eyebrow">{model.primaryExtension.toUpperCase()} model</div>
        ...
        <div className="primary-actions">
          ...
          <button className="button button--quiet" disabled={!activeAsset || activeAsset.missing} onClick={() => activeAsset && api.revealAsset(activeAsset.id)}><FolderOpen size={16} /> Reveal</button>
        </div>
        <div className="inspector-scroll">
          <DetailSection title="Details" icon={<Scale size={16} />}>
            <dl className="metadata-list"><div><dt>Dimensions</dt><dd>{formatDimensions(activeAsset?.metadata.dimensionsMm ?? viewerGeometry?.dimensionsMm ?? model.dimensionsMm)}</dd></div><div><dt>File size</dt><dd>{activeAsset ? formatBytes(activeAsset.byteSize) : "—"}</dd></div><div><dt>Modified</dt><dd>{formatDate(activeAsset?.modifiedAt)}</dd></div><div><dt>Format</dt><dd>{activeAsset?.extension.toUpperCase()}</dd></div>{(activeAsset?.metadata.triangleCount ?? viewerGeometry?.triangleCount ?? 0) > 0 && <div><dt>Triangles</dt><dd>{(activeAsset?.metadata.triangleCount || viewerGeometry?.triangleCount || 0).toLocaleString()}</dd></div>}</dl>
          </DetailSection>
          {model.webSource && <DetailSection title={`Source · ${model.webSource.provider === "makerworld" ? "MakerWorld" : "Printables"}`} icon={<Globe2 size={16} />}><div className="model-web-source">{model.webSource.imageUrl && <img src={model.webSource.imageUrl} alt="" referrerPolicy="no-referrer" />}<div><strong>{model.webSource.title}</strong>{model.webSource.creator && <small>by {model.webSource.creator}</small>}{model.webSource.license && <span>{model.webSource.license}</span>}{model.webSource.filamentGrams != null && <span>{model.webSource.filamentGrams.toFixed(1)} g · default print profile</span>}</div><button className="button button--quiet" onClick={() => api.openExternal(model.webSource!.canonicalUrl)}><ExternalLink size={14} /> Open source</button></div></DetailSection>}
          {activeAsset?.metadata.threeMf && <DetailSection title={`3MF plates & profile · ${activeAsset.metadata.threeMf.plates?.length ?? 0}`} icon={<Printer size={16} />}><ThreeMfInspector assetId={activeAsset.id} metadata={activeAsset.metadata.threeMf} selectedPlateIndex={activePlateIndex} onSelectPlate={setActivePlateIndex} onShowAll={() => setActivePlateIndex(undefined)} /></DetailSection>}
          <DetailSection title={`Files & variants · ${model.assets.length}`} icon={<FileBox size={16} />}>
            <div className="asset-list">{model.assets.map((asset) => <button key={asset.id} onClick={() => setActiveAssetId(asset.id)} className={asset.id === activeAsset?.id ? "is-active" : ""}><span className="file-icon">{asset.extension.toUpperCase()}</span><span><strong>{asset.filename}</strong><small>{formatBytes(asset.byteSize)} · {titleCase(asset.parseStatus)}</small></span>{asset.id === activeAsset?.id && <Check size={15} />}</button>)}</div>
          </DetailSection>
          <DetailSection title={`Tags${model.tags.length ? ` · ${model.tags.length}` : ""}`} icon={<Tags size={16} />}><TagEditor modelId={model.id} selected={model.tags} /></DetailSection>
          <DetailSection title="Collections" icon={<Box size={16} />}><CollectionPicker modelId={model.id} selected={model.collectionIds} /></DetailSection>
          {related.length > 0 && <DetailSection title={`Copies & versions · ${related.length}`} icon={<GitBranch size={16} />}><div className="related-models">{related.map((item) => <button key={item.id} onClick={() => selectModel(item.id)}><span className={`related-models__icon is-${item.relationship}`}>{item.relationship === "duplicate" ? <Copy size={14} /> : <GitBranch size={14} />}</span><span><strong>{item.displayName}</strong><small>{item.relationship === "duplicate" ? "Exact duplicate" : "Possible version"} · {item.relativeFolder || "Library root"}</small></span><span className="file-pill">{item.primaryExtension.toUpperCase()}</span></button>)}</div></DetailSection>}
          <DetailSection title="Material cost" icon={<Scale size={16} />}><CostCalculator modelId={model.id} assetId={activeAsset?.id} suggestedGrams={suggestedFilamentGrams} suggestedSource={suggestedFilamentSource} estimate={model.estimate} /></DetailSection>
          <DetailSection title="Notes" icon={<FileBox size={16} />} open={Boolean(model.notes)}>
            <textarea className="notes-field" value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Add print settings, assembly notes, or anything useful…" />
            <button className="button button--quiet button--full" disabled={notes === model.notes} onClick={async () => { await api.saveNotes(model.id, notes); queryClient.invalidateQueries({ queryKey: ["model", model.id] }); }}><Save size={15} /> Save notes</button>
          </DetailSection>
        </div>
      </aside>
    </div>
